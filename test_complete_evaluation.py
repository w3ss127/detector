...
class ComprehensiveModelTester:
    ...
    def load_models(self, autoencoder_checkpoint_path=None, classifier_path='noise_classifier_model.joblib'):
        ...
        if os.path.exists(classifier_path):
            ...
            logger.warning(f"⚠️  Classifier file not found: {classifier_path}")
            ...
            logger.warning("⚠️  Continuing without classifier - only autoencoder features will be available")
            self.classifier = None
            self.noise_analyzer = None
    ...
    def evaluate_comprehensive(self, test_tensors: torch.Tensor, test_labels: torch.Tensor):
        ...
        # Step 2: Normalize features and make predictions (if classifier available)
        if self.classifier is not None and self.noise_analyzer is not None:
            ...
            logger.warning("⚠️  Skipping classification - classifier not available")
            predictions = None
            probabilities = None
            normalized_features = None
            main_pbar.update(2)  # Skip both normalization and prediction steps
