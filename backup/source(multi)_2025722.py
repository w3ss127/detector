...
def setup_logging(log_dir, log_file, rank=0):
    """Configure logging for each rank."""
    os.makedirs(log_dir, exist_ok=True)
    log_file = log_file.replace(".log", f"_rank{rank}.log")
    logging.basicConfig(
        level=logging.INFO,
...
def get_transforms(is_training=False):
    """Get data transformations for preprocessing and augmentation."""
    transforms_list = [
...
class LargeTensorDataset(Dataset):
    ...
    def __init__(self, base_dir, transform=None, rank=0, is_test=False):
        ...
        for class_idx, class_name in enumerate(Config.CLASS_NAMES):
            ...
            class_files = sorted(glob.glob(os.path.join(class_dir, "*.pt")))
            pt_files.extend([(class_idx, f) for f in class_files])

        if not pt_files:
            if self.rank == 0:
                logging.error(f"No .pt files found in {base_dir}")
            raise FileNotFoundError(f"No .pt files found in {base_dir}")

        # Load image paths and labels
        for class_idx, pt_file in tqdm(pt_files, desc=f"Loading images ({'test' if is_test else 'train'})", disable=rank != 0):
            try:
...
def train(model, loader, optimizer, criterion, scaler, rank, epoch, run_id):
    ...
    loader = tqdm(loader, desc=f"Training Epoch {epoch + 1}", disable=rank != 0)
    ...
    dist.reduce(torch.tensor(running_loss, device=rank), dst=0, op=dist.ReduceOp.SUM)
    if rank == 0:
        ...
        logging.info(f"Train Per-Class MCC: " + ", ".join(
    ...
    return 0.0, 0.0, 0.0, [0.0] * Config.NUM_CLASSES

def validate(model, loader, criterion, rank, epoch, run_id):
    ...
    all_labels = []
    dataset_len = len(loader.dataset)
    loader = tqdm(loader, desc=f"Validation Epoch {epoch + 1}", disable=rank != 0)
    with torch.no_grad():
        for x, y in loader:
            x, y = x.to(rank), y.to(rank)
            with autocast('cuda'):
    ...
    if rank == 0:
        ...
        logging.info(f"Validation Per-Class MCC: " + ", ".join(
            [f"{name}: {mcc:.4f}" for name, mcc in zip(Config.CLASS_NAMES, per_class_mcc)]))
        with open(os.path.join(Config.LOG_DIR, f"metrics_{run_id}.csv"), "a") as f:
...
def test(model, loader, rank, run_id, epoch=None):
    """Test the model on the test dataset."""
    model.eval()
    all_preds = []
    all_labels = []
    desc = f"Testing Epoch {epoch + 1}" if epoch is not None else "Testing"
    loader = tqdm(loader, desc=desc, disable=rank != 0)
    with torch.no_grad():
        for x, y in loader:
            x, y = x.to(rank), y.to(rank)
            outputs = model(x)
    ...
    if rank == 0:
        ...
        logging.info(f"Test Per-Class MCC: " + ", ".join(
            [f"{name}: {mcc:.4f}" for name, mcc in zip(Config.CLASS_NAMES, per_class_mcc)]))
        with open(os.path.join(Config.LOG_DIR, f"metrics_{run_id}.csv"), "a") as f:
    ...
    return 0.0, 0.0, [0.0] * Config.NUM_CLASSES, Config.CLASS_NAMES
...
def main(rank, world_size):
    """Main training function for distributed or single-GPU training."""
    ...
    if is_distributed:
        run_id_tensor = torch.tensor([int(run_id[i:i+2], 16) for i in range(0, len(run_id), 2)], dtype=torch.uint8, device=rank)
        dist.broadcast(run_id_tensor, src=0)
        run_id = ''.join([f'{x:02x}' for x in run_id_tensor.tolist()])
    log_file = f"training_{run_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    setup_logging(Config.LOG_DIR, log_file, rank)
    ...
    test_dataset = LargeTensorDataset(Config.TEST_DIR, transform=val_transform, rank=rank, is_test=True)
    test_sampler = torch.utils.data.distributed.DistributedSampler(test_dataset, shuffle=False) if is_distributed else None
    test_loader = DataLoader(test_dataset, batch_size=Config.BATCH_SIZE, sampler=test_sampler,
                             shuffle=False if not is_distributed else None,
                             num_workers=Config.NUM_WORKERS, pin_memory=True, prefetch_factor=2, persistent_workers=True)

    train_dataset = LargeTensorDataset(Config.TRAIN_DIR, transform=train_transform, rank=rank, is_test=False)
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_subset, shuffle=True) if is_distributed else None
    val_sampler = torch.utils.data.distributed.DistributedSampler(val_subset, shuffle=False) if is_distributed else None
    train_loader = DataLoader(train_subset, batch_size=Config.BATCH_SIZE, sampler=train_sampler,
                             shuffle=False if is_distributed else True,
                             num_workers=Config.NUM_WORKERS, pin_memory=True, prefetch_factor=2, persistent_workers=True)
    val_loader = DataLoader(val_subset, batch_size=Config.BATCH_SIZE, sampler=val_sampler,
    ...
    # Initialize model
    model = ResNetViTHybrid(Config.NUM_CLASSES).to(rank)
    ...
    if rank == 0:
        ...
        if latest_checkpoint:
            ...
            if frozen and start_epoch >= Config.FREEZE_EPOCHS:
                model.module.unfreeze_backbone() if is_distributed else model.unfreeze_backbone()
    ...
    try:
        for epoch in range(start_epoch, Config.NUM_EPOCHS):
            ...
            if epoch == Config.FREEZE_EPOCHS and not (model.module.resnet.conv1.weight.requires_grad if is_distributed else model.resnet.conv1.weight.requires_grad):
                model.module.unfreeze_backbone() if is_distributed else model.unfreeze_backbone()
                optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=Config.FINETUNE_LR, weight_decay=0.0001)
                scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=5)
                if rank == 0:
            ...
            train_loss, train_acc, train_mcc, train_per_class_mcc = train(model, train_loader, optimizer, criterion, scaler, rank, epoch, run_id)
            val_loss, val_acc, val_mcc, val_per_class_mcc = validate(model, val_loader, criterion, rank, epoch, run_id)
            ...
            if rank == 0:
                ...
                checkpoint = {
                    ...
                    'frozen': not (model.module.resnet.conv1.weight.requires_grad if is_distributed else model.resnet.conv1.weight.requires_grad),
                    'run_id': run_id
                }
        ...
        test(model, test_loader, rank, run_id)
        if is_distributed:
            cleanup_distributed()
        cleanup_logging()
