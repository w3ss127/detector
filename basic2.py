...
def main():
    ...
    test_dataset = LargeTensorDataset(TEST_DIR, transform=transform)
    ...
    if latest_checkpoint:
        try:
            ...
            logging.info(f"Resuming from checkpoint: {latest_checkpoint}, Epoch {start_epoch} (Run ID: {run_id})")
            print(f"Resuming from checkpoint: {latest_checkpoint}, Epoch {start_epoch} (Run ID: {run_id})")
        except Exception as e:
    ...
    train_dataset = LargeTensorDataset(TRAIN_DIR, transform=transform)
    ...
    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, sampler=train_sampler, num_workers=12, pin_memory=True)
    val_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, sampler=torch.utils.data.SubsetRandomSampler(val_indices, generator=torch.Generator().manual_seed(42)), num_workers=12, pin_memory=True)
    check_dataset(train_dataset, train_loader, "Train")

    logging.info(f"Train subset size: {train_size}")
    logging.info(f"Train subset distribution: " + ", ".join([f"{name}={sum(1 for i in train_indices if train_dataset.labels[i] == idx)}" 
                                                           for idx, name in enumerate(CLASS_NAMES)]))
    logging.info(f"Validation subset size: {len(val_indices)}")
    ...
    print(f"Train subset distribution: " + ", ".join([f"{name}={sum(1 for i in train_indices if train_dataset.labels[i] == idx)}" 
                                                     for idx, name in enumerate(CLASS_NAMES)]))
    print(f"Validation subset size: {len(val_indices)}")
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        train_loss, train_acc, train_mcc, train_per_class_mcc = train(model, train_loader, optimizer, criterion, scaler)
        ...
        val_loss, val_acc, val_mcc, val_per_class_mcc = validate(model, val_loader, criterion)
