...
try:
    ...
    print("⚠️ PyWavelets not available - wavelet features will be disabled")
import warnings
import gc
import psutil
import tempfile
...
def get_memory_usage() -> float:
    """Return current process RSS memory usage in GB (safe fallback to 0.0)."""
    try:
...
def get_disk_space(path: str = '/tmp') -> float:
...
def configure_safe_multiprocessing():
    ...
    try:
        ...
        if tmp_space < 1.0:  # Less than 1GB
            print(f"⚠️ /tmp has low space ({tmp_space:.1f}GB), configuring alternative temp directory")
            
            # Try alternative temp directories
            alt_temp_dirs = ['/mnt/data/temp', os.path.expanduser('~/temp')]
            for temp_dir in alt_temp_dirs:
                try:
                    os.makedirs(temp_dir, exist_ok=True)
                    space = get_disk_space(temp_dir)
                    if space > 5.0:
                        os.environ['TMPDIR'] = temp_dir
                        os.environ['TEMP'] = temp_dir
                        os.environ['TMP'] = temp_dir
                        print(f"✅ Using alternative temp directory: {temp_dir} ({space:.1f}GB available)")
                        break
                except Exception:
                    continue
        ...
        print(f"⚠️ Warning configuring multiprocessing: {e}")

# Configure safe multiprocessing at module level
...
class UltraAdvancedFeatureExtractor:
    ...
    def __init__(self, n_jobs=-1, use_gpu_features=True):
        ...
        # Check disk space and adjust parallel processing
        tmp_space = get_disk_space('/tmp')
        if tmp_space < 2.0:  # Less than 2GB in /tmp
            print(f"⚠️ Low disk space in /tmp ({tmp_space:.1f}GB), using single-threaded processing")
            self.n_jobs = 1
        else:
    ...
    def extract_comprehensive_features(self, noise_map: np.ndarray) -> np.ndarray:
        ...
        if current_length < target_length:
            ...
            # Return zero features to maintain consistency
            return np.zeros(500, dtype=np.float32)
    
    def extract_batch_features_parallel(self, noise_maps_batch: List[np.ndarray]) -> List[np.ndarray]:
        """Extract features from a batch using optimized parallel processing with timeout"""
        batch_size = len(noise_maps_batch)
        
        # Force single-threaded for large batches or low disk space
        tmp_space = get_disk_space('/tmp')
        ...
        try:
            ...
            print(f"⚠️ Parallel processing failed ({e}), falling back to single-threaded")
            return [self.extract_comprehensive_features(nm) for nm in noise_maps_batch]
...
def main_ultra_advanced():
    ...
    # Skip autoencoder training - load pre-trained model instead
    print(f"\n🔄 SKIPPING autoencoder training - loading pre-trained model...")
    
    # Try to load existing autoencoder checkpoint
    autoencoder_loaded = False
    checkpoint_names_to_try = [
        'best_autoencoder',
        'autoencoder_final',
        'autoencoder_complete',
        'latest'
    ]
    
    for checkpoint_name in checkpoint_names_to_try:
        try:
            loaded, _ = pipeline.load_checkpoint(checkpoint_name)
            if loaded and pipeline.autoencoder_trained:
                print(f"✅ Successfully loaded pre-trained autoencoder from: {checkpoint_name}")
                autoencoder_loaded = True
                break
        except Exception as e:
            print(f"⚠️ Could not load checkpoint {checkpoint_name}: {e}")
            continue
    
    if not autoencoder_loaded:
        print("⚠️ No pre-trained autoencoder found. Using randomly initialized autoencoder...")
        print("   Note: This may result in lower performance. Consider training an autoencoder first.")
        # Mark as trained to proceed with the pipeline
