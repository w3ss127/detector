...
class SuperiorConfig:
    """Enhanced configuration specifically for semi-synthetic detection"""
    def __init__(self):
        # Model Configuration
        ...
        self.FOCAL_ALPHA = [1.0, 6.0, 2.5]
        self.FOCAL_GAMMA = 3.5
        self.LABEL_SMOOTHING = 0.15
        self.CLASS_WEIGHTS = [1.0, 4.0, 2.0]
...
class UncertaintyModule(nn.Module):
    """Module for uncertainty quantification and confidence estimation"""
...
class SuperiorLoss(nn.Module):
    """Advanced loss function designed for semi-synthetic detection"""
    ...
    def __init__(self, config):
        ...
        self.focal_alpha = torch.tensor(config.FOCAL_ALPHA).to(config.DEVICE)
...
class SuperiorAttentionModule(nn.Module):
    """Enhanced attention module with forensics-aware attention"""
...
class SuperiorModel(nn.Module):
    """Superior model with advanced forensics-aware architecture"""
...
class SuperiorDataset(Dataset):
    """Enhanced dataset with intelligent sampling and class-specific augmentations"""
    ...
    def _load_file_mapping(self):
        logger.info("Creating/loading superior file mapping...")
            ...
            for class_idx, class_name in enumerate(self.class_names):
                ...
                for pt_file in pt_files:
                    try:
                        ...
                        logger.warning(f"Skipping corrupted file {pt_file}: {e}")
        ...
        logger.info(f"Found {len(self.file_indices)} images")
    ...
    def _balance_dataset(self):
        ...
        logger.info(f"Balanced dataset: {len(self.file_indices)} samples")
        new_class_counts = np.bincount(self.labels)
        for i, count in enumerate(new_class_counts):
            logger.info(f"Balanced class {self.class_names[i]}: {count} samples")
...
def calculate_superior_metrics(y_true, y_pred, y_probs=None, class_names=None):
    ...
    if len(cm) >= 3:
        total_semi = cm[1].sum()
        correct_semi = cm[1, 1]
        confused_with_real = cm[1, 0]
        confused_with_synthetic = cm[1, 2]
        semi_confusion_rate = (confused_with_real + confused_with_synthetic) / total_semi if total_semi > 0 else 0
    else:
        semi_confusion_rate = 0
...
def evaluate_superior_model(model, data_loader, criterion, config, device, epoch=1):
    model.eval()
    total_loss = 0
    all_predictions = []
    all_targets = []
    ...
    metrics = calculate_superior_metrics(
        all_targets, all_predictions, all_probabilities, config.CLASS_NAMES
    )
...
def create_superior_optimizer(model, config, epoch):
    ...
    if epoch >= config.FINE_TUNE_START_EPOCH:
        ...
        logger.info(f"Fine-tuning SGD optimizer (epoch {epoch})")
    else:
        ...
        logger.info(f"AdamW optimizer (epoch {epoch})")
...
def save_superior_checkpoint(model, optimizer, scaler, epoch, metrics, config, filename, is_best=False):
    checkpoint = {
        'epoch': epoch,
...
def plot_confusion_matrix(cm, class_names, save_path):
    ...
    plt.title('Confusion Matrix - Superior Model', fontsize=16, fontweight='bold')
    plt.ylabel('True Label', fontsize=14)
    plt.xlabel('Predicted Label', fontsize=14)
    
    total = np.sum(cm)
    accuracy = np.trace(cm) / total
    plt.figtext(0.15, 0.02, f'Overall Accuracy: {accuracy:.4f}', fontsize=12)
    
    for i, class_name in enumerate(class_names):
        if i < len(cm):
            class_acc = cm[i, i] / cm[i].sum() if cm[i].sum() > 0 else 0
            plt.figtext(0.15, 0.06 - i*0.02, f'{class_name} Accuracy: {class_acc:.4f}', fontsize=10)
...
def superior_train_worker(local_rank, config, master_port):
    ...
    try:
        ...
        logger.info(f"Superior training setup complete for rank {local_rank}")
        ...
        if local_rank in [-1, 0]:
            ...
            logger.info(f"Starting superior training with {initial_trainable:,} trainable parameters")
        ...
        for epoch in range(config.EPOCHS):
            ...
            if local_rank in [-1, 0]:
                val_loss, val_metrics, val_preds, val_targets, val_probs = evaluate_superior_model(
                    model, val_loader, criterion, config, config.DEVICE, epoch + 1
                )
                ...
                logger.info(f"Val Loss: {val_loss:.4f}, Val Acc: {val_metrics['accuracy']:.4f}, Val MCC: {val_metrics['mcc']:.4f}")
                logger.info(f"Semi-synthetic - Precision: {val_metrics['semi_synthetic_precision']:.4f}, "
                          f"Recall: {val_metrics['semi_synthetic_recall']:.4f}, F1: {val_metrics['semi_synthetic_f1']:.4f}")
                logger.info(f"Semi-synthetic Confusion Rate: {val_metrics['semi_confusion_rate']:.4f}")
                ...
                if current_score > best_score:
                    ...
                    save_superior_checkpoint(model, current_optimizer, scaler, epoch, 
                                           val_metrics, config, best_model_path, is_best=True)
                    epochs_no_improve = 0
                else:
                    epochs_no_improve += 1
                ...
                plot_superior_metrics(training_history, 
                                    os.path.join(config.CHECKPOINT_DIR, f'superior_metrics_epoch_{epoch+1}.png'))
                
                cm = confusion_matrix(val_targets, val_preds)
                plot_confusion_matrix(cm, config.CLASS_NAMES, 
                                    os.path.join(config.CHECKPOINT_DIR, f'superior_cm_epoch_{epoch+1}.png'))
        ...
        if local_rank in [-1, 0]:
            logger.info("\n" + "="*60)
            logger.info("SUPERIOR TRAINING COMPLETED - FINAL EVALUATION")
            logger.info("="*60)
            ...
            test_loss, test_metrics, test_preds, test_targets, test_probs = evaluate_superior_model(
                model, test_loader, criterion, config, config.DEVICE, epoch + 1
            )
            ...
            logger.info(f"Precision: {test_metrics['semi_synthetic_precision']:.4f}")
            logger.info(f"Recall: {test_metrics['semi_synthetic_recall']:.4f}")
            logger.info(f"F1-Score: {test_metrics['semi_synthetic_f1']:.4f}")
            logger.info(f"Confusion Rate: {test_metrics['semi_confusion_rate']:.4f}")
            ...
            for i, row in enumerate(cm):
                logger.info(f"{config.CLASS_NAMES[i]}: {row}")
            ...
            for i, class_name in enumerate(config.CLASS_NAMES):
                if i < len(test_metrics['per_class_precision']):
                    ...
                    logger.info(f"  F1-Score: {test_metrics['per_class_f1'][i]:.4f}")
            
            logger.info(f"\nTRAINING SUMMARY:")
            logger.info(f"Total epochs: {epoch+1}")
            logger.info(f"Best validation semi-synthetic F1: {best_metrics['semi_synthetic_f1']:.4f}")
            logger.info(f"Best validation MCC: {best_metrics['mcc']:.4f}")
            logger.info(f"Final test semi-synthetic F1: {test_metrics['semi_synthetic_f1']:.4f}")
            logger.info(f"Final test MCC: {test_metrics['mcc']:.4f}")
            ...
            logger.info(f"Forensics module: {'Enabled' if config.USE_FORENSICS_MODULE else 'Disabled'}")
            logger.info(f"Uncertainty estimation: {'Enabled' if config.USE_UNCERTAINTY_ESTIMATION else 'Disabled'}")
        ...
        if local_rank in [-1, 0]:
            logger.info("Superior training completed successfully!")
        
        return training_history
        
    except Exception as e:
        logger.error(f"Error in superior_train_worker for rank {local_rank}: {e}")
        if config.DISTRIBUTED:
            cleanup_distributed()
        cleanup_memory()
        raise
...
def main():
    ...
    parser.add_argument('--no_forensics', action='store_true',
                        help='Disable forensics-aware module')
